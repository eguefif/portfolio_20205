...
from src.html_utils import generate_tech_badges_html, generate_youtube_link_html
...
def main():
    ...
    for i in range(1, 5):
        ...
        # Generate modal HTML
        modal_html = generate_modal_html(
            modal_template,
            i,
            project_data["title"],
            project_data["tech"],
            project_data["modal_content"],
        )
        ...
        output = output.replace(
            f"{{{{ project_{i}_description }}}}", project_data["description"]
        )
    ...
    if args.publish:
        publish_to_website(
            "./index.html", "/home/eguefif/lab/personal_websites/personal_website"
        )
